...
static void read_hr_log( struct CWMCU_T *sensor )
{
	...
	{
		memset( data, 0, 40 );
        if (CWMCU_I2C_R(sensor, RegMapR_hrLog_Event, data, 30) >= 0)
		{
			parser_hr_log(data);
		}
        else
...
{  
   int ret = 0 ; 
   ret = mt_set_gpio_out(GPIO_CW_MCU_RESET, 1); 
   if( ret <0  )
   	  SH_LOG("gpio set  GPIO_CW_MCU_RESET  1 error  \n");
   
   ret = mt_set_gpio_out(GPIO_CW_MCU_BOOT, 1);  
   if( ret <0  )
   	  SH_LOG("gpio set  GPIO_CW_MCU_BOOT error  \n");
   msleep(100);    
   
   ret = mt_set_gpio_out(GPIO_CW_MCU_BOOT, bootmode);    
   if( ret <0  )
...
static int exec_change_orientation(struct device *dev, int leftRight)
{
	...
	if (sensor->meg_status[1] == 0) {
		get_mcu_sensors_info(sensor, emMAGNETIC_FIELD_SENSOR, sensor->meg_status);
		printk("[CHECK] MEG chip address = 0x%02x\n", sensor->meg_status[1]);
	}
	SH_LOG("cwm: is_mercury=%d, has_nfc=%d MEG chip address = 0x%02x\n", is_mercury, has_nfc, sensor->meg_status[1]);
...
static ssize_t set_firmware_update_cmd(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sensor->cmd == SET_SENSORS_POSITION && CW_BOOT == sensor->mcu_mode) {
		SH_LOG("mcu_mode = boot \n");
	...
	switch (sensor->cmd) {
		...
		case CHANGE_TO_NORMAL_MODE:
            ...
            SH_LOG(" Start(5 0 0)(CHANGE_TO_NORMAL_MODE) block_orientation=%d\n", block_orientation);

			if (block_orientation == 1) {
				SH_LOG("cywee modify watch orientation.\n");
				exec_change_orientation(dev, sensor->watch_orientation);
				block_orientation = 0;
			}
			break;
		...
		case SET_SENSORS_POSITION:
			...
			printk("CWM: is_mercury = %d, has_nfc = %d, MEG address = 0x%02x\n", (int)is_mercury, (int)has_nfc, sensor->meg_status[1]);
			if (is_mercury == false) {
				if (id == MAGNETIC) {
					if (sensor->len == 5)
						sensor->len = 4;
...
static ssize_t get_suspend_flag(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct CWMCU_T *sensor = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", sensor->mcu_suspend_flag);
}

static ssize_t set_suspend_flag(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct CWMCU_T *sensor = dev_get_drvdata(dev);
	sscanf(buf, "%d\n", &sensor->mcu_suspend_flag);
	return count;
...
static int get_chip_id(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct CWMCU_T *sensor = dev_get_drvdata(dev);
    uint8_t status[31] = {0};
	int len = 0;
	...
	len += sprintf(buf ,"[ACCELEROMETER] ChipId:[%02x] Addres:[%02x] HwId:[%02x] HwType:[%02x] Calib[%02x] SelfTest[%02x]\n",
		status[0],status[1],status[2],status[3],sensor->CalibratorStatus[emACCELEROMETER],sensor->SelfTestStatus[emACCELEROMETER]);

	get_mcu_sensors_info(sensor,emMAGNETIC_FIELD_SENSOR,status);
	len += sprintf(buf+len,"[MAGNETIC] ChipId:[%02x] Addres:[%02x] HwId:[%02x] HwType:[%02x] Calib[%02x] SelfTest[%02x]\n",
		status[0],status[1],status[2],status[3],sensor->CalibratorStatus[emMAGNETIC_FIELD_SENSOR],sensor->SelfTestStatus[emMAGNETIC_FIELD_SENSOR]);

	get_mcu_sensors_info(sensor,emGYROSCOPE,status);
	len += sprintf(buf+len,"[GYROSCOPE] ChipId:[%02x] Addres:[%02x] HwId:[%02x] HwType:[%02x] Calib[%02x] SelfTest[%02x]\n",
		status[0],status[1],status[2],status[3],sensor->CalibratorStatus[emGYROSCOPE],sensor->SelfTestStatus[emGYROSCOPE]);

	get_mcu_sensors_info(sensor,emHEART_RATE,status);
	len += sprintf(buf+len,"[HEART_RATE] ChipId:[%02x] Addres:[%02x] HwId:[%02x] HwType:[%02x] Calib[%02x] SelfTest[%02x]\n",
		status[0],status[1],status[2],status[3],sensor->CalibratorStatus[emHEART_RATE],sensor->SelfTestStatus[emHEART_RATE]);
...
static struct device_attribute attributes[] = {
	__ATTR(enable, S_IRUGO|S_IWUSR|S_IWGRP, active_show, active_set),
	__ATTR(batch, S_IWUSR|S_IWGRP, NULL, batch_set),
	__ATTR(flush, S_IWUSR|S_IWGRP, NULL, flush_set),
	__ATTR(mcu_mode, S_IRUGO|S_IWUSR|S_IWGRP, mcu_mode_show, mcu_model_set),
	__ATTR(calibrator_cmd, S_IRUGO|S_IWUSR|S_IWGRP, get_calibrator_cmd, set_calibrator_cmd),
	__ATTR(calibrator_data, S_IRUGO|S_IWUSR|S_IWGRP, get_calibrator_data, set_calibrator_data),
	__ATTR(firmware_update_i2c, S_IRUGO|S_IWUSR|S_IWGRP, get_firmware_update_i2, set_firmware_update_i2),
	__ATTR(firmware_update_cmd, S_IWUSR|S_IWGRP, NULL, set_firmware_update_cmd),
    ...
    __ATTR(sys_msg, S_IRUGO|S_IWUSR|S_IWGRP, get_sys_msg, set_sys_msg),
	...
	__ATTR(chip_id, S_IRUGO, get_chip_id, NULL),
	__ATTR(suspend_flag, S_IRUGO|S_IWUSR|S_IWGRP, get_suspend_flag, set_suspend_flag),
};
...
static void CWMCU_early_suspend(struct early_suspend *h);
static void CWMCU_late_resume(struct early_suspend *h);
static struct notifier_block cw_fb_notifier;
static int cw_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct fb_event *evdata = data;
	INT32 blank;

	CW_INFO("cw_fb_notifier_callback\n");

	if (event != FB_EVENT_BLANK)
		return 0;

	blank = *(INT32 *)evdata->data;
	CW_INFO("fb_notify(blank=%d)\n", blank);
...
static void cwmcu_error_handle_report(struct work_struct *work)
{
    ...
    if(sensor->i2c_error_state == CWM_LEVEL1_MCU_RESET){
        ...
        mutex_unlock(&sensor->mutex_lock);
        if (sensor->mcu_mode == CW_BOOT) {
            SH_ERR("want to change to CW_NORMAL, but now mode is CW_BOOT\n");
        }
        sensor->mcu_mode = CW_NORMAL;
        if (CWMCU_I2C_R(sensor, RegMapR_ChipId, data, 2) >= 0)
        {
...
#if defined(CONFIG_FB)
	cw_fb_notifier.notifier_call = cw_fb_notifier_callback;
	if (fb_register_client(&cw_fb_notifier))
		CW_ERROR("register fb_notifier fail!\n");
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
#if defined(CONFIG_FB)
	fb_unregister_client(&cw_fb_notifier);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
