...
static long cw_factory_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
        ...
        case GSENSOR_IOCTL_GET_CALI:
            ...
            if (copy_to_user(data, &sensor_data, sizeof(sensor_data))!=0);
                CW_ERROR("[FACTORY] GSENSOR_IOCTL_GET_CALI copy_to_user failed");
            break;
        ...
        case GYROSCOPE_IOCTL_GET_CALI:
            ...
            if (copy_to_user(data, &sensor_data, sizeof(sensor_data)) != 0)
                CW_ERROR("[FACTORY] GYROSCOPE_IOCTL_GET_CALI copy_to_user failed");
            break;
		...
		case ALSPS_GET_ALS_RAW_DATA:
			...
			//}
			CW_DEBUG("[FACTORY] CW_LIGHT data[]={%d %d %d}", data_event[0], data_event[1], data_event[2]);
			if(copy_to_user(data, &data_event[0], sizeof(int)))
			{
				err = -EFAULT;
				break;	  
